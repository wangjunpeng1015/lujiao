...
function createOrder() {
    ...
    data.append('appId', getUserParam('appId'))
...
function countdown(time) {
    if (!isInPay) {
        endTime = dayjs(time).add(5, 'm')
        isInPay = true
        let siv = setInterval(function () {
            diffTime = endTime.diff(dayjs(), 's')
            let modulo = diffTime % (60 * 60 * 24) % (60 * 60);
            //分钟
