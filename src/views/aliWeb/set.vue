...
.layout-column
  ...
  el-dialog(
    ...
    el-form(:model='news' ref='news', label-width='120px')
      el-form-item(label='账号备注：', prop='account')
        el-input(v-model='news.account' placeholder="请填写收款账号备注")
  ...
  .funds-header.layout-row__between
    div
      el-button(v-if="userinfo.roleId == 4||userinfo.roleId == 1" type="primary" size="mini" @click="dialogShow = true") 添加账号
  ...
  methods: {
    ...
    getAllAcount() {
      ...
      getAllAcount({
        ...
        param: {
          code: this.code, //码商
          account: `${this.account}-aliweb`, //账号
          // account: "-aliscan", //账号
          used: this.used, //是否启用
          accountType: "ali", //类型
          min: this.min, //最小
    ...
    saveAccount() {
      ...
      let account = this.news.account + "-aliweb";
      let param = Object.assign({}, this.news, {
        account: this.news.account + "-aliweb"
      });
      addAcount(param)
        .then(res => {
          this.getAllAcount();
