...
.layout-column
  ...
  el-dialog(
    ...
    el-form(:model='news' ref='news', label-width='120px')
      el-form-item(label='TJ账号：', prop='account')
        el-input(v-model='news.account' placeholder="请填写TJ账号")
  ...
  .funds-header.layout-row__between
    el-button(v-if="userinfo.roleId == 4||userinfo.roleId == 1" type="primary" size="mini" @click="dialogShow = true") 添加TJ账号
    el-form(label-width='120px' :inline="true" size="mini")
      ...
      el-form-item
        el-input(v-model='account' placeholder="请输入TJ账号")
      el-form-item
  ...
  methods: {
    ...
    saveAccount() {
      ...
      let param = Object.assign({}, this.news, {
        account: `${this.news.account}-TJ红包`
      });
