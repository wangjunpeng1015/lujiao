...
.layout-column
  ...
  el-dialog(
    ...
    el-form(:model='news' ref='news', label-width='120px')
      el-form-item(label='探街账号：', prop='account')
        el-input(v-model='news.account' placeholder="请填写探街账号")
  ...
  .funds-header.layout-row__between
    el-button(v-if="userinfo.roleId == 4||userinfo.roleId == 1" type="primary" size="mini" @click="dialogShow = true") 添加探街账号
    el-form(label-width='120px' :inline="true" size="mini")
      ...
      el-form-item
        el-input(v-model='account' placeholder="请输入探街账号")
      el-form-item
  ...
  methods: {
    ...
    saveAccount() {
      ...
      let param = Object.assign({}, this.news, {
        account: `${this.news.account}-探街红包`
      });
